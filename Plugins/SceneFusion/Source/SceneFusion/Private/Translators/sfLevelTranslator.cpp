...
void sfLevelTranslator::PreTick(float deltaTime)
{
    ...
    for (ULevel* levelPtr : m_localActorOrderChangedLevels)
    {
        ...
        auto iter = m_objectToProperty.find(levelObjPtr);
        if (iter == m_objectToProperty.end())
        {
            continue;
        }
...
void sfLevelTranslator::Tick(float deltaTime)
{
    ...
    for (ULevel* levelPtr : m_dirtyParentLevels)
    {
        ...
        auto iter = m_objectToProperty.find(objPtr);
...
void sfLevelTranslator::UpdateActorOrder()
{
    for (ULevel* levelPtr : m_serverActorOrderChangedLevels)
    {
        ...
        sfListProperty::SPtr actorListPtr = iter->second->Property()->AsDict()->Get(sfProp::Actors)->AsList();
...
void sfLevelTranslator::OnCreateLevelObject(sfObject::SPtr objPtr)
{
    ...
    if (!isPersistentLevel)
    {
        ...
        if (levelPtr != nullptr)
        {
            ...
            if (m_levelsNeedToBeLoaded.find(objPtr) == m_levelsNeedToBeLoaded.end())
            {
            ...
            if (streamingLevelPtr != nullptr)
            {
                ...
                if (m_worldPtr->GetWorldSettings()->bEnableWorldComposition)
                {
                    ...
                    {
                        sfPropertyManager::Get().ApplyProperties(worldTileDetailsObjPtr, levelPropertiesPtr);
                    }
                }
                ...
                sfPropertyManager::Get().ApplyProperties(streamingLevelPtr, levelPropertiesPtr, &PROPERTY_BLACKLIST);

                // Set references to this streaming level
                std::vector<sfReferenceProperty::SPtr> references = m_sessionPtr->GetReferences(propertyObjPtr);
                ...
                {
                    for (sfObject::SPtr childPtr : propertyObjPtr->Children())
                    {
    ...
    if (propertyObjPtr->Property()->AsDict()->HasKey(sfProp::Actors))
    {
...
ULevel* sfLevelTranslator::TryLoadLevelFromFile(FString levelPath, bool isPersistentLevel)
{
...
void sfLevelTranslator::UploadLevel(ULevel* levelPtr)
{
    ...
    sfObject::SPtr propertyObjPtr = nullptr;

    if (levelPtr->IsPersistentLevel())
    {
        AWorldSettings* worldSettingsPtr = m_worldPtr->GetWorldSettings();
        propertyObjPtr = sfObjectMap::GetOrCreateSFObject(worldSettingsPtr, sfType::LevelProperties);
        levelPropertiesPtr = propertyObjPtr->Property()->AsDict();
        ...
        InitializeChildren(propertyObjPtr);
    ...
    else
    {
        ...
        if (streamingLevelPtr != nullptr)
        {
            propertyObjPtr = sfObjectMap::GetOrCreateSFObject(streamingLevelPtr, sfType::LevelProperties);
            levelPropertiesPtr = propertyObjPtr->Property()->AsDict();
        ...
        {
            propertyObjPtr = sfObject::Create(sfType::LevelProperties, sfDictionaryProperty::Create());
            levelPropertiesPtr = propertyObjPtr->Property()->AsDict();
        }
...
void sfLevelTranslator::OnAddLevelToWorld(ULevel* newLevelPtr, UWorld* worldPtr)
{
    ...
    if (m_unloadedLevelObjects.RemoveAndCopyValue(levelPath, levelObjPtr))
    {
        ...
        {
            KS::Log::Error("Cannot find ULevelStreaming object for level " + sfUnrealUtils::FToStdString(levelPath),
                LOG_CHANNEL);
        }
    }
    else
...
void sfLevelTranslator::RegisterPropertyChangeHandlers()
{
    ...
    {
        auto iter = m_propertyToObject.find(propertyPtr->GetContainerObject());
        if (iter != m_propertyToObject.end())
        {
            ULevel* levelPtr = sfObjectMap::Get<ULevel>(iter->second);
            if (levelPtr != nullptr)
            {
                m_serverActorOrderChangedLevels.Add(levelPtr);
            }
    ...
    m_propertyChangeHandlers[sfProp::LevelPropertyId] = [this](UObject* uobjPtr, sfProperty::SPtr propertyPtr)
    {
        // The level property id never actually changes so we don't need to do anything, but the change event is fired
        // when the uobject the property references is created, so we need a handler to avoid getting a warning saying
...
bool sfLevelTranslator::OnUPropertyChange(sfObject::SPtr objPtr, UObject* uobjPtr, UProperty* upropPtr)
{
...
void sfLevelTranslator::OnTileDetailsChange(UObject* uobjPtr, UProperty* upropPtr)
{
    sfObject::SPtr levelObjPtr;
