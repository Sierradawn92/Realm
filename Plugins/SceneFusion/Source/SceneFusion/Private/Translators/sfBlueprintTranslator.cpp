...
{
    if (blueprintPtr == nullptr || sfObjectMap::Contains(blueprintPtr))
    {
        return;
    }
    ...
    if (blueprintPtr->GeneratedClass != nullptr)
    {
        ...
        if (defaultObjectPtr != nullptr)
        {
            ...
            for (USCS_Node* rootNodePtr : blueprintPtr->SimpleConstructionScript->GetRootNodes())
            {
...
void sfBlueprintTranslator::OnCreateBlueprint(sfObject::SPtr objPtr)
{
    ...
    UBlueprint* blueprintPtr = LoadObject<UBlueprint>(nullptr, *name);
