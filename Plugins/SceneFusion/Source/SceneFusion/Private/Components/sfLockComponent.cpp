...
void UsfLockComponent::InitializeComponent()
{
    ...
    FEditorDelegates::PreSaveWorld.AddUObject(this, &UsfLockComponent::OnPreSave);
    FEditorDelegates::PostSaveWorld.AddUObject(this, &UsfLockComponent::OnPostSave);
}
...
{
    AActor* actorPtr = GetOwner();
    if (actorPtr != nullptr && !actorPtr->IsA<ALandscapeStreamingProxy>())
    {
...
void UsfLockComponent::OnPreSave(uint32_t saveFlags, UWorld* worldPtr)
{
    AActor* actorPtr = GetOwner();
    if (actorPtr != nullptr && !actorPtr->IsA<ALandscapeStreamingProxy>())
    {
...
void UsfLockComponent::OnPostSave(uint32_t saveFlags, UWorld* worldPtr, bool success)
{
    AActor* actorPtr = GetOwner();
    if (actorPtr != nullptr)
    {
