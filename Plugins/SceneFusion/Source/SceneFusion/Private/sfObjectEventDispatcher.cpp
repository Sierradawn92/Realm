...
void sfObjectEventDispatcher::Register(const sfName& objectType, TSharedPtr<sfBaseTranslator> translatorPtr)
{
...
bool sfObjectEventDispatcher::OnUPropertyChange(sfObject::SPtr objPtr, UObject* uobjPtr, UProperty* upropPtr)
{
    if (objPtr == nullptr)
    {
        return false;
...
void sfObjectEventDispatcher::PostPropertyChange(UObject* uobjPtr, UProperty* upropPtr)
{
    sfObject::SPtr objPtr = sfObjectMap::GetSFObject(uobjPtr);
...
{
    if (type == sfType::Blueprint && !sfConfig::Get().SyncBlueprint)
    {
        return nullptr;
    }
