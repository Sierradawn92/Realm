...
#include "Translators/sfComponentTranslator.h"
#include "Translators/sfLevelTranslator.h"
#include "Translators/sfMeshStandInTranslator.h"
#include "Translators/sfUObjectTranslator.h"
#include "Translators/sfBlueprintTranslator.h"
#include "Translators/sfModelTranslator.h"
...
void SceneFusion::StartupModule()
{
    ...
    ObjectEventDispatcher->Register(sfType::Asset, MakeShareable(new sfAssetTranslator));
    ObjectEventDispatcher->Register(sfType::UObject, MakeShareable(new sfUObjectTranslator));
...
void SceneFusion::OnObjectsReplaced(const TMap<UObject*, UObject*>& replacementMap)
{
    ...
    for (auto actorPtr : actorPtrs)
    {
        ...
        if (objPtr != nullptr && objPtr->IsLocked())
        {
            ...
            actorTranslatorPtr->Lock(actorPtr, sfObjectMap::GetSFObject(actorPtr));
        }
    }
}
