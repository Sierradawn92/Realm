...
sfUPropertyInstance sfPropertyManager::FindUProperty(UObject* uobjPtr, sfProperty::SPtr propPtr)
{
    ...
    UProperty* containerUPropPtr = nullptr;
    ...
    while (stack.size() > 0)
    {
        ...
        if (!GetArrayElement(propPtr->Index(), upropPtr, ptr) &&
            !GetFixedArrayElement(propPtr->Index(), upropPtr, ptr) &&
            !GetStructField(propPtr->Key(), upropPtr, ptr) &&
    ...
    return sfUPropertyInstance(upropPtr, ptr, mapPtr, setPtr, containerUPropPtr);
}

bool sfPropertyManager::GetStructField(const sfName& name, UProperty*& upropPtr, void*& ptr)
{
...
bool sfPropertyManager::GetArrayElement(int index, UProperty*& upropPtr, void*& ptr)
{
...
bool sfPropertyManager::GetFixedArrayElement(int index, UProperty* upropPtr, void*& ptr)
{
...
sfProperty::SPtr sfPropertyManager::GetValue(UObject* uobjPtr, UProperty* upropPtr)
{
    ...
    {
        return iter->second.Get(sfUPropertyInstance(upropPtr, upropPtr->ContainerPtrToValuePtr<void>(uobjPtr)));
    }
    sfListProperty::SPtr listPtr = sfListProperty::Create();
    for (int i = 0; i < upropPtr->ArrayDim; i++)
    {
        listPtr->Add(iter->second.Get(
            sfUPropertyInstance(upropPtr, upropPtr->ContainerPtrToValuePtr<void>(uobjPtr, i))));
    }
    return listPtr;
}
...
bool sfPropertyManager::IsDefaultValue(UObject* uobjPtr, UProperty* upropPtr)
{
    if (uobjPtr == nullptr ||
...
void sfPropertyManager::SetToDefaultValue(UObject* uobjPtr, UProperty* upropPtr)
{
    if (uobjPtr == nullptr || upropPtr == nullptr)
    {
        return;
    ...
    for (TFieldIterator<UProperty> iter(uobjPtr->GetClass()); iter; ++iter)
    {
        ...
        if (IsSyncable(uobjPtr, upropPtr))
        {
            ...
            else
            {
                ...
                if (SetValue(uobjPtr,
                    sfUPropertyInstance(upropPtr, iter->ContainerPtrToValuePtr<void>(uobjPtr)), propPtr))
                {
...
void sfPropertyManager::MarkPropertyChanged(UObject* uobjPtr, UProperty* upropPtr, sfProperty::SPtr propPtr)
{
    if (uobjPtr == nullptr)
    {
        return;
...
void sfPropertyManager::BroadcastChangeEvents()
{
    ...
    for (TPair<UObject*, TSet<UProperty*>>& pair : m_serverChangedProperties)
    {
        ...
        for (UProperty* propPtr : pair.Value)
        {
...
{
    TSet<UProperty*>* propertiesPtr = m_localChangedProperties.Find(oldPtr);
...
void sfPropertyManager::SyncProperties()
{
    ...
    for (TPair<UObject*, TSet<UProperty*>>& pair : m_localChangedProperties)
    {
...
void sfPropertyManager::SyncProperty(sfObject::SPtr objPtr, UObject* uobjPtr, UProperty* upropPtr, bool applyServerValue)
{
...
void sfPropertyManager::Initialize()
{
    ...
    CREATE_TYPE_HANDLER(UDoubleProperty, GetDouble, SetDouble);
    CREATE_TYPE_HANDLER(UStrProperty, GetFString, SetFString);
    CREATE_TYPE_HANDLER(UTextProperty, GetFText, SetFText);
    CREATE_TYPE_HANDLER(UNameProperty, GetFName, SetFName);
    CREATE_TYPE_HANDLER(UEnumProperty, GetEnum, SetEnum);
    CREATE_TYPE_HANDLER(UArrayProperty, GetArray, SetArray);
    CREATE_TYPE_HANDLER(UMapProperty, GetMap, SetMap);
    CREATE_TYPE_HANDLER(USetProperty, GetSet, SetSet);
    CREATE_TYPE_HANDLER(UStructProperty, GetStruct, SetStruct);
    CREATE_TYPE_HANDLER(UObjectProperty, GetObject, SetObject);
    CREATE_TYPE_HANDLER(USoftObjectProperty, GetSoftObject, SetSoftObject);
    CREATE_TYPE_HANDLER(ULazyObjectProperty, GetLazyObject, SetLazyObject);
    CREATE_TYPE_HANDLER(UWeakObjectProperty, GetWeakObject, SetWeakObject);
    CREATE_TYPE_HANDLER(UInterfaceProperty, GetInterface, SetInterface);
    CREATE_TYPE_HANDLER(UClassProperty, GetClass, SetClass);
    CREATE_TYPE_HANDLER(USoftClassProperty, GetSoftClass, SetSoftClass);
}
...
bool sfPropertyManager::IsSyncable(UObject* uobjPtr, UProperty* upropPtr)
{
...
bool sfPropertyManager::IsPropertyInForceSyncList(UProperty* upropPtr)
{
...
sfProperty::SPtr sfPropertyManager::GetStruct(const sfUPropertyInstance& upropInstance)
{
    ...
    UProperty* subPropPtr = tPtr->Struct->PropertyLink;
    while (subPropPtr)
    {
...
{
    UWeakObjectProperty* tPtr = Cast<UWeakObjectProperty>(upropInstance.Property());
    FWeakObjectPtr& weakObjPtr = *(FWeakObjectPtr*)upropInstance.Data();
    if (propPtr->Type() == sfProperty::NUL)
    {
        if (!weakObjPtr.IsValid())
...
{
    UInterfaceProperty* tPtr = Cast<UInterfaceProperty>(upropInstance.Property());
...
bool sfPropertyManager::SetReference(const sfUPropertyInstance& upropInstance, UObject* referencePtr)
{
    ...
    if (USoftObjectProperty* tPtr = Cast<USoftObjectProperty>(upropInstance.Property()))
    {
        if (referencePtr == tPtr->GetObjectPropertyValue(upropInstance.Data()))
        {
            return false;
    ...
    if (ULazyObjectProperty* tPtr = Cast<ULazyObjectProperty>(upropInstance.Property()))
    {
        if (referencePtr == tPtr->GetObjectPropertyValue(upropInstance.Data()))
        {
            return false;
    ...
    if (UWeakObjectProperty* tPtr = Cast<UWeakObjectProperty>(upropInstance.Property()))
    {
        if (referencePtr == tPtr->GetObjectPropertyValue(upropInstance.Data()))
        {
            return false;
    ...
    if (UInterfaceProperty* tPtr = Cast<UInterfaceProperty>(upropInstance.Property()))
    {
        FScriptInterface inter = tPtr->GetPropertyValue(upropInstance.Data());
...
void sfPropertyManager::GetInstances(UObject* defaultObjPtr, TArray<UObject*>& instances)
{
    ...
    else if (outerPtr->HasAnyFlags(RF_ClassDefaultObject | RF_ArchetypeObject))
    {
        ...
        for (UProperty* Property = outerPtr->GetClass()->RefLink; Property != nullptr; Property = Property->NextRef)
        {
